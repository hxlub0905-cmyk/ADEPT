# CLAUDE.md 的完整版（2026-09-09 封存）

> **這一份是 2026-09-09 瘦身之前的 `CLAUDE.md`，逐字保留。** 那一份每個 session
> 都會被讀進去，而它長到 647 行，一半是「某年某月使用者說了什麼」的故事 ——
> 規則留在 `CLAUDE.md`，故事搬到這裡。要查「這條規矩當初是怎麼踩出來的」，
> 搜這一份；要查現在的規矩，讀 `CLAUDE.md`。連結已改成從 `docs/history/` 出發。
